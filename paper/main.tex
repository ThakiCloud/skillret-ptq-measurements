...
costliest in the Qwen lineage and EmbeddingGemma while attention dominates BGE-M3, and the
interaction residual varies in both magnitude and sign. At INT2/g16 checkpoints whose
reconstruction error is clustered between 0.315 and 0.337 retain between 1.3\% and 65.7\% of
...
($r=0.87$ to $0.92$ in four of the five checkpoints) but tracks damage from quantizing
...
themselves explain the cliff: quantizing only the embedding table to INT2 retains 98.0\% to
100.5\% of retrieval quality in the per-model averages (95.6\% to 101.3\% across individual
...
module for Qwen3, its fine-tuned derivative and EmbeddingGemma, BGE-M3 reverses the ordering,
and E5 is effectively tied and becomes attention-dominant at g32 --- and module costs stop
...
98.0\% to 100.5\% across all five checkpoints, while the dominant isolated damage shifts to
...
Weight reconstruction error tracks the damage from uniform quantization well ($r=0.87$ to
$0.92$ in four of five checkpoints, Spearman $\rho=1.00$ in three) and tracks the damage from
quantizing individual modules poorly ($r=-0.01$ to $0.42$ across fixed operating points; pooling
...
necessity undecided, so we report the cliff as unexplained. The prompt-hash, byte-ledger and
...
but the ten that do not are simply small effects measured on 4{,}392 queries, and three of
...
leaves six, and the largest effect is unchanged at 1.01 points --- the same global maximum
...
Qwen3-Embedding, its fine-tuned derivative, and EmbeddingGemma, the feed-forward block is the
most expensive module to quantize. For BGE-M3 it is the cheapest: the FFN costs 0.01 points
while attention costs 2.46. The inversion is not an artifact of one setting --- at INT3/g32
BGE-M3's attention costs 5.09 points against 1.14 for its FFN, the largest single-module
effect anywhere in our grid. A policy that says ``protect the feed-forward block'' is right
for three of our five checkpoints --- Qwen3, its fine-tuned derivative and EmbeddingGemma ---
is effectively tied on E5, and spends its extra bits on the wrong tensors on BGE-M3; at g32
E5 becomes attention-dominant as well.
...
$-1.91$ for its fine-tune, $-0.80$ for BGE-M3 and $-0.35$ for EmbeddingGemma --- joint
quantization is worse than the parts predict --- but $+0.32$ for E5, where it is
\emph{better}. At INT3/g32 every model is negative, from $-0.40$ to $-2.12$. So the
interaction is substantial in all five models and at both operating points, its magnitude
varies by a factor of five, and its sign is not even stable across models at one setting. Treating
...
embedding table to INT2 leaves 98.0\% to 100.5\% of retrieval quality in all five
...
predictive power by what is being varied.

Along the uniform axis it works, and the relationship is monotone rather than merely linear:
Spearman $\rho$ is $1.00$ for three of the five checkpoints and $0.90$ for a fourth, while
Pearson $r$ runs $0.87$ to $0.92$. E5-base-v2 appears to be an exception at $r=0.56$, but its
bootstrap interval is $[-0.24, +1.00]$ --- uninformative. Excluding collapsed arms leaves E5
with almost no dynamic range, since its surviving arms all sit at small losses; including
them raises its $r$ to $0.97$, the highest of the five. The apparent exception is a
restricted-range artifact of the exclusion rule, not a property of the model. For uniform
bit-width screening, the proxy is informative.
...
fixed removes the leak. Across the evaluated checkpoints the correlation is $-0.008$ at
INT4/g16, $0.367$ at INT3/g16, $0.408$ at INT3/g32 and $0.415$ at INT2/g16, against $0.86$ for
...
further, to $0.91$ or above in every model, while leaving the module arms where they are. The failure is structural rather
than noisy. BGE-M3's \texttt{int3/g32} arm and its FFN-only variant at the same bit width
differ by $0.015$ in reconstruction error, in the direction that would predict the FFN-only
arm to be \emph{worse}; the measured retrieval losses are 13.2\% and 2.4\%. Quantizing more
of the model moved the weights less and hurt retrieval far more.
...
arithmetic estimate. The arithmetic prediction runs 0.04 to 0.44\% above the packed file across the seven quantized artifacts, so the two are close but not interchangeable; the measured byte count is what we report, and \texttt{artifacts.csv} in the measurement repository carries the file name, byte count and SHA-256 behind every size in this paper.
...
vocabulary the survivors sit at both extremes --- E5-base-v2 at 30{,}522 keeps 41.9\% and
EmbeddingGemma at 262{,}144 keeps 65.7\% --- while BGE-M3 at 250{,}002 keeps 1.8\%. BGE-M3 and
EmbeddingGemma have similarly large vocabularies, 250{,}002 and 262{,}144, yet their INT2
retention differs by 63.9 points.
...
the cliff, quantizing that table alone should reproduce substantial degradation. It does not. Embedding-only INT2 leaves 98.0\% to 100.5\% of retrieval quality
...
negative --- $-8.6$ points for EmbeddingGemma and $-12.2$ for E5-base-v2 --- so joint
...
65.7\% of its full-precision NDCG, E5 keeps 41.9\%, and the remaining three keep 1.3\% to
...
spans $0.315$ to $0.337$ retain between 1.3\% and 65.7\% of their retrieval quality.
